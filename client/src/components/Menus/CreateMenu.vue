<template>
  <div style="padding:20px;max-width:600px;margin:0 auto;">
    ...
    <form v-on:submit.prevent="createMenu">
      ...
      <div style="margin-bottom:15px;">
        <button type="submit" :disabled="loading" style="padding:10px 20px;font-size:16px;margin-right:10px;background-color:#4CAF50;color:white;border:none;border-radius:5px;cursor:pointer;">
...
export default {
  ...
  methods: {
    ...
    async createMenu () {
      ...
      try {
        console.log('Creating menu with data:', this.menu)
        const response = await MenuService.post(this.menu)
        console.log('Menu created successfully:', response.data)
        this.success = 'บันทึกเมนูสำเร็จ!'
